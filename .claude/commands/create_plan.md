...
Decompose the work into phases — do not ask the user to iterate on the phase structure. Each phase should be a thin vertical slice that type-checks and (where applicable) keeps existing tests green. For each phase, decide:
- Phase name
- Goal (one sentence)

Keep the phase list tight; merge anything that does not stand alone.

This repository is a TypeScript monorepo (npm workspaces + Turborepo). The relevant workspaces live under `apps/*` and `packages/*` — primarily `apps/cli`, `apps/mcp`, `apps/extension`, and `packages/tm-core`. Respect the layering rule in `CLAUDE.md`: all business logic belongs in `@tm/core`; CLI/MCP/extension are thin presentation layers. Phase decomposition should reflect that — e.g. land core changes first, then wire the presentation layers.
...
- [ ] Typecheck passes: `npm run turbo:typecheck`
- [ ] Unit tests pass for the touched package(s): `npm run test -w <package-name>` (e.g. `npm run test -w @tm/core`)
- [ ] Format check passes: `npm run format-check`
- [ ] [Other phase-specific automated check, e.g. a new spec file added]
...
- [ ] [Specific CLI command or MCP tool behavior covering Phase 1]
- [ ] [Specific behavior covering Phase 2]
- [ ] [End-to-end behavior matching the design's success criteria]

## Manual Testing Steps
1. [step — e.g. `npm run turbo:build` then run `task-master <subcommand>`]
2. [step]
