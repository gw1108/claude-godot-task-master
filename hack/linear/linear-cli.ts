...
const needsAuth =
	...
	!['--help', '-h', '--version', '-v', 'completion', 'help'].includes(
		process.argv[2]
	);
...
if (needsAuth) {
	if (!process.env.LINEAR_API_KEY) {
		console.error(
			chalk.red('Error: Missing LINEAR_API_KEY environment variable')
		);
		console.error(
			chalk.yellow('Please set it with: export LINEAR_API_KEY=your_api_key')
		);
		process.exit(1);
	}
...
async function getIssueIdInteractively(
	...
	const { issueId } = await inquirer.prompt({
		...
		default: defaultId,
		validate: (input) => {
			return /^[A-Za-z0-9]+-\d+$/i.test(input)
				? true
				: 'Please enter a valid issue ID (e.g. ENG-123, B2BPROD-206)';
		}
	});
...
async function getIssue(issueId?: string) {
	try {
		...
		console.error(
			chalk.red('Error fetching issue:'),
			error instanceof Error ? error.message : String(error)
		);
		process.exit(1);
	}
...
async function displayIssue(issue: any, showLinearUrl: boolean = true) {
	...
	if (comments.nodes.length > 0) {
		...
		for (const comment of reversedComments) {
			...
			console.log(
				chalk.dim(`[${dateStr} ${timeStr}] ${commentUser?.name || 'Unknown'}:`)
			);
...
async function updateStatus(
	...
	try {
		...
		if (!issueId || !/^[A-Za-z0-9]+-\d+$/i.test(issueId)) {
			console.error(
				chalk.red(
					'Error: Invalid issue ID format. Expected format: ENG-123 or B2BPROD-206'
				)
			);
			process.exit(1);
		}
		...
		if (!team) {
			console.error(
				chalk.red(`Could not determine team for issue ${normalizedId}.`)
			);
			process.exit(1);
		}
		...
		if (!targetState) {
			console.error(
				chalk.red(`Status "${statusName}" not found for team ${team.name}.`)
			);
		...
		if (result.success) {
			console.log(
				chalk.green(`✓ Updated ${normalizedId} status to "${targetState.name}"`)
			);
		} else {
		...
		console.error(
			chalk.red('Error updating status:'),
			error instanceof Error ? error.message : String(error)
		);
		process.exit(1);
	}
...
async function assignToMe(issueId?: string): Promise<void> {
	try {
		...
		if (!viewer) {
			console.error(
				chalk.red('Error: Could not get authenticated user information.')
			);
			process.exit(1);
		}
		...
		if (result.success) {
			console.log(
				chalk.green(
					`✓ Assigned ${resolvedId} to ${viewer.name || viewer.email}`
				)
			);
		...
		console.error(
			chalk.red('Error assigning issue:'),
			error instanceof Error ? error.message : String(error)
		);
		process.exit(1);
	}
	...
	try {
		...
		for (let i = 0; i < images.length; i++) {
			...
			try {
				...
				console.error(
					chalk.yellow(`⚠ Failed to download image ${i + 1}: ${errorMsg}`)
				);
			}
		}
		...
		console.error(
			chalk.red('Error fetching images:'),
			error instanceof Error ? error.message : String(error)
		);
		process.exit(1);
	}
...
async function getIssueV2(
	...
	try {
		...
		if (useCompactJson || usePrettyJson) {
			...
			console.log(
				JSON.stringify(cleanedData, null, usePrettyJson ? 2 : undefined)
			);
		} else {
			// Markdown format
			if (
				requestedFields.includes('identifier') &&
			...
			if (
				requestedFields.includes('parents') &&
				issueData.parents &&
				issueData.parents.length > 0
			) {
				console.log(
					chalk.bold.yellow(
						'\n═══════════════════════════════════════════════════════════════'
				...
				for (let i = 0; i < issueData.parents.length; i++) {
					...
					if (requestedFields.includes('assignee') && parent.assignee) {
						console.log(
							chalk.dim(`${levelPrefix}Assignee: ${parent.assignee}`)
						);
					}
					...
					if (
						...
						for (const comment of parent.comments) {
							...
							console.log(
								chalk.dim(
									`${levelPrefix}[${dateStr} ${timeStr}] ${comment.author}:`
								)
							);
...
async function listIssuesV2(options: {
	...
	try {
		...
		const fieldsExplicitlyProvided =
			options.fields && options.fields !== defaultFields;

		// Validate mutually exclusive options
		if (idsOnly && fieldsExplicitlyProvided) {
			console.error(
				chalk.red('Error: --ids-only and --fields cannot be used together')
			);
			process.exit(1);
		}
		...
		if (options.assignee) {
			...
			const matchingUser = users.nodes.find(
				(user) =>
			...
			if (!matchingUser) {
				console.error(
					chalk.red(`Error: No user found matching "${assigneeValue}"`)
				);
				process.exit(1);
			}
			...
			for (const issue of issuesData) {
				...
				if (
					...
					for (const comment of issue.comments) {
						...
						const timeStr = commentDate.toTimeString().split(' ')[0];
						console.log(
							chalk.dim(`  [${dateStr} ${timeStr}] ${comment.author}:`)
						);
...
program
	...
	.description(
		'Get a single issue with the same output format and field options as list-issues'
	)
	.option(
		'--output-format <format>',
		'Output format: markdown, json (compact), or rich-json (pretty)',
		'json'
	)
	.option(
		'--fields <fields>',
		'Comma-separated fields to include: identifier,title,branch,assignee,description,comments,parents',
		'identifier,title,branch'
	)
	.action(getIssueV2);
...
program
	...
	.description(
		'Add a comment to an issue (auto-detects issue ID from git branch)'
	)
...
program
	...
	.description(
		'Download all images from a Linear issue to thoughts/shared/images/'
	)
	.action(fetchImages);
...
program
	...
	.description(
		"Update the status of a Linear issue (e.g. 'research needed', 'in review')"
	)
	.action(updateStatus);
...
program
	...
	.option(
		'-t, --title <title>',
		'Optional title for the link (defaults to URL)'
	)
	.action(addLink);
...
program
	...
	.option(
		'--assignee <assignee>',
		'Filter by assignee (name, email, or display name)'
	)
	.option(
		'--status <statuses>',
		"Filter by status (comma-separated, e.g. 'research needed,in review')"
	)
	.option(
		'--size <sizes>',
		"Filter by issue size (comma-separated numbers, e.g. '1,2')"
	)
	.option(
		'--sort-asc',
		'Sort by updatedAt in ascending order (default is descending)'
	)
	.option(
		'--output-format <format>',
		'Output format: markdown, json (compact), or rich-json (pretty)',
	...
	.option(
		'--fields <fields>',
		'Comma-separated fields to include: identifier,title,branch,assignee,description,comments',
		'identifier,title,branch'
	)
	.option(
		'--ids-only',
		'Output only issue identifiers (incompatible with --fields)'
	)
	.action(listIssuesV2);
...
complete -c linear -n "__fish_seen_subcommand_from completion" -l fish -d "Generate Fish completion script"`);
			...
			console.error(
				chalk.red('Please specify a shell: --bash, --zsh, or --fish')
			);
			process.exit(1);
		}
	});
